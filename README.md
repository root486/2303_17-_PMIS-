...
### 6. docs（文档目录）
存放项目相关的补充文档。README.md 为项目说明文档，涵盖目录结构、功能模块、技术栈与运行说明；init_database.sql 为数据库初始化脚本，包含部门表、员工表、工作经历表、公告表、请假表的建表语句及测试数据，一步到位完成数据库环境准备。便于团队成员查阅项目背景和设计细节，也方便后续维护和迭代。
...
### 8. bin（可执行文件目录，可选）
存放项目的可执行脚本。init_database.sql 用于一次性初始化全部数据库表结构与测试数据（含部门、员工、工作经历、公告、请假表），首次部署时执行即可完成数据库环境准备。
...
| 请假管理 | 员工自主申请请假，管理者审批，按姓名/类型/状态搜索、批量删除 |
...
1. 数据库初始化：在 MySQL 中执行 init_database.sql（一键初始化全部表结构 + 测试数据）；
